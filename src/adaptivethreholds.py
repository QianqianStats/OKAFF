...
class OneSidedAdaptiveThreshold:
    """One-sided adaptive threshold based on moments of ``statistic**2``.

    This is the common threshold used by NEWMA, MMDEW, and Online RFF MMD.
    """
    ...
    def __init__(
        ...
        self.multiplier = float(norm.ppf(quantile))
        self.t0 = int(t0)
        self.require_nonnegative = bool(require_nonnegative)
        self.reset()

    def update(self, statistic: float) -> bool:
        """Update the threshold and return whether this time step alarms."""
        value = _validate_statistic(
            statistic,
        ...
        squared = value**2
        self.second_moment = (
            (1.0 - self.rho) * self.second_moment
            + self.rho * squared
        )
        self.fourth_moment = (
            (1.0 - self.rho) * self.fourth_moment
            + self.rho * squared**2
        )
        self.squared_variance = max(
            0.0, self.fourth_moment - self.second_moment**2
        )
        upper_squared = max(
            0.0,
            self.second_moment
            + self.multiplier * sqrt(self.squared_variance),
        )
        self.threshold = sqrt(upper_squared)

        self.alarm = bool(
            self.time >= self.t0 and value > self.threshold
        )
        return self.alarm
    ...
    def reset(self) -> None:
        ...
        self.time = 0
        self.second_moment = 0.0
        self.fourth_moment = 0.0
        self.squared_variance = 0.0
        self.threshold = 0.0
        self.alarm = False


class NEWMAAdaptiveThreshold(OneSidedAdaptiveThreshold):
    """NEWMA threshold with ``rho`` tied to NEWMA's slow ``lambda``."""
    ...
    def __init__(
        ...
        super().__init__(rho=slow_lambda, quantile=quantile, t0=t0)
        self.slow_lambda = self.rho


class MMDEWAdaptiveThreshold(OneSidedAdaptiveThreshold):
    """MMDEW one-sided threshold; the retained experiment uses rho=0.1."""


class OnlineRFFMMDAdaptiveThreshold(OneSidedAdaptiveThreshold):
    """Online RFF MMD one-sided threshold; default rho=0.1."""


def make_adaptive_threshold(
    detector_name: str,
    *,
    slow_lambda: Optional[float] = None,
    rate: float = 0.1,
    quantile: float = 0.95,
    t0: int = 50,
    initialization: str = "zero",
    ...
    """Construct the appropriate threshold from a detector name.
    """
    key = (
        detector_name.strip()
        .lower()
    ...
    if key == "newma":
        if slow_lambda is None:
            raise ValueError("slow_lambda is required for NEWMA")
        return NEWMAAdaptiveThreshold(
            slow_lambda=slow_lambda, quantile=quantile, t0=t0
        )
    if key == "mmdew":
        return MMDEWAdaptiveThreshold(
            rho=rate, quantile=quantile, t0=t0
        )
    if key in {"onlinerffmmd", "streamingrffmmd"}:
        return OnlineRFFMMDAdaptiveThreshold(
            rho=rate, quantile=quantile, t0=t0
        )
    raise ValueError(
        "Unknown detector_name. Expected OKAFF, NEWMA, MMDEW, "
        "or OnlineRFFMMD."
    )
...
__all__ = [
    ...
    "OneSidedAdaptiveThreshold",
    "NEWMAAdaptiveThreshold",
    "MMDEWAdaptiveThreshold",
    "OnlineRFFMMDAdaptiveThreshold",
