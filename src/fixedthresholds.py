...
def default_band_lambda(algo_kwargs):
    """Read the theory-band lambda using the experiment's fallback order."""
    if algo_kwargs is None:
        algo_kwargs = {}
    return float(
        algo_kwargs.get(
            "band_lambda", algo_kwargs.get("lambda0", 1.0 - 1e-3)
        )
    )
