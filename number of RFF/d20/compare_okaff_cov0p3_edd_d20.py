...
EDD/d20/results when the plotting script is stored elsewhere.
"""
...
def load_case(results_dir: Path, *, features: int, n_runs: int) -> pd.DataFrame:
    path = results_dir / f"OKAFF-ARL-EDD-d20-m{features}-nrun{n_runs}.csv"
    if not path.is_file():
    ...
    required = {"post_change_name", "num_features_m", "arl_hat", "edd_hat"}
