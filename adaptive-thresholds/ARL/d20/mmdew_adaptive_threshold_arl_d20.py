...
OUTPUT_DIR = Path(__file__).resolve().parent / "results" / f"MMDEW-adaptive-thresholds-ARL-d{D}-nrun{N_RUNS}"

def make_detector_and_thresholds(reference):
    """Construct MMDEW and initialize it with observations 1,...,50."""
...
def simulate_one_run(*, run_index=0):
...
def save_checkpoint(completed_runs, run_lengths, censored):
    """Atomically save all results after each completed Monte Carlo run."""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    summary_file = OUTPUT_DIR / f"MMDEW-adaptive-thresholds-ARL-d{D}.csv"
    summary_temporary = OUTPUT_DIR / f"MMDEW-adaptive-thresholds-ARL-d{D}.csv.tmp"
...
def estimate_arl():
    ...
    for run in range(N_RUNS):
        lengths_one_run, censored_one_run = simulate_one_run(run_index=run)
        for q in QUANTILES:
...
if __name__ == "__main__":
    ...
    print("\nMMDEW adaptive-threshold ARL under N(0, I_d)")
    ...
    print(f"\nSaved: {OUTPUT_DIR / f'MMDEW-adaptive-thresholds-ARL-d{D}.csv'}")
