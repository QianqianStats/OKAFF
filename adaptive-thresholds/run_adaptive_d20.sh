...
run_stage() {
    local stage="$1"
    local folder="$RESULTS_DIR/$stage/d20"
...
echo "Comparing d20 ARL and EDD results"
cd "$RESULTS_DIR"
"$PYTHON_BIN" -u "$RESULTS_DIR/compare_arl_edd.py" --dimension 20 --results-dir "$RESULTS_DIR" --n-runs "$N_RUNS"
