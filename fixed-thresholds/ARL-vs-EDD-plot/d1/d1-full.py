...
TITLE_MAP = {
    'MeanShift(mu=1)': r'$\mathcal{N}(1,1)$',
    'MeanShift(mu=2)': r'$\mathcal{N}(2,1)$',
    'MeanShift(mu=3)': r'$\mathcal{N}(3,1)$',
    'MeanShift(mu=4)': r'$\mathcal{N}(4,1)$',
    'CovDiag(var=0.3)': r'$\mathcal{N}(0,0.3)$',
    'CovDiag(var=0.5)': r'$\mathcal{N}(0,0.5)$',
    'CovDiag(var=2)': r'$\mathcal{N}(0,2)$',
    'CovDiag(var=6)': r'$\mathcal{N}(0,6)$',
    'MixedNormal0.1': r'$\mathrm{MixedNormal}(0.1)$',
    'MixedNormal0.2': r'$\mathrm{MixedNormal}(0.2)$',
    'MixedNormal0.3': r'$\mathrm{MixedNormal}(0.3)$',
    'MixedNormal0.4': r'$\mathrm{MixedNormal}(0.4)$',
    'Laplace(scale=0.5)': r'$\operatorname{Laplace}(0,0.5)$',
    'Laplace(scale=1.0)': r'$\operatorname{Laplace}(0,1.0)$',
    'Laplace(scale=3.0)': r'$\operatorname{Laplace}(0,3.0)$',
    'Laplace(scale=4.0)': r'$\operatorname{Laplace}(0,4.0)$',
    'Uniform(scale=0.5)': r'$\operatorname{Uniform}([-0.5,0.5])$',
    'Uniform(scale=1.0)': r'$\operatorname{Uniform}([-1.0,1.0])$',
    'Uniform(scale=3.0)': r'$\operatorname{Uniform}([-3.0,3.0])$',
    'Uniform(scale=4.0)': r'$\operatorname{Uniform}([-4.0,4.0])$',
}
...
def postchange_row_name(row_datas, row_idx):
    ...
    if first.startswith("MeanShift(mu="):
        return "edd_vs_arl_dense_mean_shift_d1.pdf"
    if first.startswith("MeanShift(first5coord="):
        return "edd_vs_arl_sparse_mean_shift_d1.pdf"
    if first.startswith("CovDiag(var="):
        return "edd_vs_arl_dense_variance_shift_d1.pdf"
    if first.startswith("CovDiag(first5var="):
        return "edd_vs_arl_sparse_variance_shift_d1.pdf"
    if first.startswith("MixedNormal"):
        return "edd_vs_arl_gaussian_mixture_d1.pdf"
    if first.startswith("Laplace("):
        return "edd_vs_arl_laplace_d1.pdf"
    if first.startswith("Uniform("):
        return "edd_vs_arl_uniform_d1.pdf"
    return f"edd_vs_arl_row{row_idx:02d}_d1.pdf"
...
dfp = load_results()
saved_paths = save_row_plots(dfp, output_dir="arl_edd_d1_full")
