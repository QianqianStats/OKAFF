...
def estimate_rl_moments_h0_streaming_rffmmd_tau(
    tau: float,
    *,
    d: int,
    ref_size: int,
...
if __name__ == "__main__":
    ...
    # ARL from 100-1300
    thresholds = [0.98, 1.04, 1.05]
    # full ARL from 100-18000
    # thresholds = [0.98, 1.00, 1.04, 1.05, 1.07, 1.09, 1.1, 1.11, 1.115, 1.12]
    ...
    for tau in thresholds:
        metrics = estimate_rl_moments_h0_streaming_rffmmd_tau(
            tau,
            d=d,
            ref_size=ref_size,
        ...
        row = {
            ...
            **metrics,
            "n_runs": int(n_runs),
            "burn_in_stats": int(burn_in_stats),
            "d": int(d),
            "ref_size": int(ref_size),
            "num_omegas": int(num_omegas),
        }
        rows.append(row)
    ...
    out_csv = out_dir / f"online_rff_mmd-ARL-d{d}-nrun{n_runs}.csv"
