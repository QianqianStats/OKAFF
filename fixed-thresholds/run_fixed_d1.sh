...
run_plots() {
    ...
    echo "Starting d1 ARL-vs-EDD plots"
    for script in "${scripts[@]}"; do
        echo "Running $script"
        (cd "$folder" && "$PYTHON_BIN" -u "$script" --n-runs "$N_RUNS")
    done
...
run_experiments "d1 ARL experiments" "$FIXED_DIR/ARL/d1"
run_experiments "d1 EDD experiments" "$FIXED_DIR/EDD/d1"
run_plots "$FIXED_DIR/ARL-vs-EDD-plot/d1"
